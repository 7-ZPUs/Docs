...
La Search bar è invece gestita da un componente dedicato. La sua responsabilità è quella di permettere all'utente di ricercare processi tramite id di processo, classi per nome e ricerche di testo libero sfruttando la ricerca semantica.
\\ Oltre al compoenente di presentazione pure è stato realizzato un servizio dedicato alla logica del componente. Per la ricerca semantica infatti è stato aggiunto un checkbox. Alla versione attuale del codice la ricerca semantica è abilitata per la ricerca di documenti ma se in un futuro si volesse estendere questa funzionalità basterebbe modificare il servizio \textit{SearchBarRulesManager}