...
Per i documenti, è previsto un trattamento speciale: il file principale (non gli allegati) viene utilizzato per la generazione dell'embedding, che servirà successivamente per la ricerca semantica. L'estrazione del testo e la generazione dei vettori sono orchestrate da \codeword{EmbeddingService}, che interagisce con \codeword{WordEmbeddingPort} per convertire il contenuto testuale in vettori di embedding. Questi vettori vengono successivamente salvati in una apposita tabella nel database.\\
...
\begin{itemize}
    \item \textbf{Descrizione}: adapter che implementa \codeword{IPackageReaderPort}, responsabile della lettura del pacchetto DiP dal filesystem locale. Orchestr a il flusso di estrazione delle entità coordinando \codeword{DipParser}, \codeword{FileSystemPort} e \codeword{DataMapper}.
    \item \codeword{\textbf{+ readDip()}: Promise<Dip>}: legge il file \codeword{DiPIndex} dal percorso specificato, lo decodifica tramite \codeword{DipParser} e lo trasforma in entità \codeword{Dip} tramite \codeword{DataMapper}.
    \item \codeword{\textbf{+ readDocumentClasses()}: AsyncGenerator<DocumentClass>}: genera in modo asincrono le entità \codeword{DocumentClass} estratte dal pacchetto. Per ogni classe documentale presente in DipIndex.xml, ne viene eseguito il parsing e il mapping verso l'entità di dominio corrispondente tramite\codeword{DataMapper}.
    \item \codeword{\textbf{+ readProcesses()}: AsyncGenerator<Process>}: genera in modo asincrono le entità \codeword{Process} estratte dal pacchetto, leggendo e decodificando i file XML che contengono i metadati aggiuntivi tramite \codeword{DipParser} e trasformandoli tramite \codeword{DataMapper}.
    \item \codeword{\textbf{+ readDocuments()}: AsyncGenerator<Document>}: genera in modo asincrono le entità \codeword{Document} estratte dal pacchetto, gestendo la lettura dei metadati e la loro mappatura.
    \item \codeword{\textbf{+ readFiles()}: AsyncGenerator<File>}: genera in modo asincrono le entità \codeword{File} estratte dal pacchetto, distinguendo tra file principale e allegati.
    \item \codeword{\textbf{+ setDipPath(dipPath: string)}: void}: imposta il percorso del pacchetto DiP da cui leggere i dati, necessario per risolvere correttamente i percorsi relativi dei file all'interno del pacchetto.
    \item \codeword{\textbf{- getOptionalMetadata(relativePath: string)}: Promise<any>}: metodo helper che, dato un percorso relativo, legge il file corrispondente all'interno del pacchetto, ne effettua il parsing tramite \codeword{DipParser} e restituisce i dati grezzi. Utilizzato per recuperare i metadati supplementari di processi, documenti e file.
    \item \codeword{\textbf{- getParsedIndex()}: Promise<any>}: metodo helper che legge e decodifica il file \codeword{DiPIndex} tramite \codeword{DipParser} e mappato tramite \codeword{DataMapper}. Utilizzato come punto di partenza per l'estrazione delle entità del pacchetto. Questo elemento è presente in ogni pacchetto DiP.
\end{itemize}
