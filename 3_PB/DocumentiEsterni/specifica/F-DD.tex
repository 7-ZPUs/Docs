...
\subparagraph{\textit{AggregateFacade}} Come per i documenti, è presente una facade dedicata per le aggregazioni che si occupa di orchestrare i servizi necessari a recuperare i dati e i metadati dell'entità (con collegamento ai canali IPC). Inoltre implementa un sistema di \textit{Fetch Retry Synchronization} per gestire situazioni in cui i dati non sono ancora disponibili al momento della richiesta.
...
\begin{itemize}
    \item \textit{admin-procedure}: indica il procedimento a cui il fascicolo appartiene, nonché lo stato di avanzamento e le relative fasi;
    \item \textit{aggregate-metadata}: mostra i metadati specifici dell'aggregazione, come Tipo, Progressivo, Classificazione, ecc.;
    \item \textit{aggregate-subjects}: mostra i soggetti coinvolti nella produzione dell'aggregazione, con i relativi ruoli, differiscono leggermente dai soggetti dei documenti;
    \item \textit{document-index}: mostra l'indice dei documenti che compongono l'aggregazione, con collegamento alla relativa pagina di dettaglio.
\end{itemize}
...
\subparagraph{\textit{ProcessFacade}} Come per le aggregazioni, è presente una facade dedicata per i processi che si occupa di orchestrare i servizi necessari a recuperare i dati e i metadati dell'entità (con collegamento ai canali IPC). Anche in questo caso implementa un sistema di \textit{Fetch Retry Synchronization}.

\subparagraph{Componente di presentazione}
\textit{Process-Metadata-Component} in questo caso è l'unico componente specifico per i processi, mostra i metadati specifici dell'entità con un formato \textit{card-based}, lo stato di verifica e informazioni sulla classe documentale di appartenenza.
