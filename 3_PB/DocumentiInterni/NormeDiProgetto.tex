...
Il fornitore analizza i \glossario{requisiti} necessari alla proposta di fornitura, tenendo
...
\begin{itemize}
    \item \textbf{Ruoli necessari}: identificare i ruoli necessari per lo sviluppo del progetto, come ad esempio sviluppatori, \glossario{verificatori}, \glossario{analisti}, ecc.
...
In particolare, l'identificazione delle \glossario{milestone} deve tenere conto dei seguenti aspetti:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Definizione dei requisiti:} \glossario{Classificazione} dettagliata dei requisiti individuati
\end{enumerate}
...
dettagliata alla pianificazione, esecuzione e \glossario{consuntivo} delle attività
...
misurate attraverso opportuni test e \glossario{verifiche}, di cui vengono riportate le
...
supportando il lavoro dei \glossario{progettisti} e degli sviluppatori.
...
\begin{itemize}
    ...
    \item \textbf{GitHub}: per il versionamento dei file PlantUML e la gestione delle \glossario{Pull Request} di verifica dei diagrammi di dettaglio (livelli C4 3 e 4).
...
Nel caso di documenti ricorrenti come i verbali, si raggruppano più documenti omogenei in un unico \glossario{CI} (e.g. "Verbali").
...
\begin{itemize}
    \item \textbf{Approvazione}: Il revisore procede con l'approvazione delle modifiche e la chiusura della PR tramite l'opzione "Create Merge Commit". Inoltre, specifica nello smart \glossario{commit} la direttiva "\#done" per chiudere automaticamente la relativa task su Jira.
          \begin{figure}[H]
              \label{PR2}
              \centering
...
\metricdef{MPD-1}{Requisiti \glossario{obbligatori} soddisfatti}
...
\metricdef{MPD-2}{Requisiti \glossario{opzionali} soddisfatti}
...
\metricdef{MPD-3}{Requisiti \glossario{desiderabili} soddisfatti}
