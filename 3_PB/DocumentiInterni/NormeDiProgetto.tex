...
\paragraph{Docker (Dev Containers)}
\begin{itemize}
    \item \textbf{Giustificazione (Funzionalità e Prestazioni):} Permette di standardizzare l'ambiente di sviluppo, garantendo che tutti i membri lavorino con le stesse dipendenze e configurazioni, riducendo problemi di "it works on my machine".
    \item \textbf{Configurazione:} Utilizzo di Dev Containers per definire un ambiente di sviluppo preconfigurato, con tutte le estensioni e strumenti necessari già installati, facilitando l'onboarding di nuovi membri e la coerenza tra gli ambienti di sviluppo.
\end{itemize}

\paragraph{SonarQube}
\begin{itemize}
    \item \textbf{Giustificazione (Funzionalità e Prestazioni):} Strumento di analisi statica del codice che aiuta a identificare problemi di qualità, vulnerabilità e code smells, migliorando la qualità del codice. Necessario per il raccoglimento di metriche come code,statement e branch coverage, code smells ecc..
    \item \textbf{Configurazione:} Integrazione con GitHub Actions per eseguire l'analisi ad ogni commit, con report dettagliati visualizzati direttamente nella piattaforma GitHub.
\end{itemize}
...
\subparagraph{\LaTeX}
Per l'utilizzo di \LaTeX è necessario che ogni membro del gruppo utilizzi i seguenti strumenti (presenti anche nei Dev Containers):
\begin{itemize}
...
\subparagraph{Docker (Dev Containers)}
L'implementazione dei Dev Containers richiede la 
configurazione di Docker su ogni macchina personale. I passi fondamentali sono:

\begin{enumerate}
    \item \textbf{Installazione di Docker:} Ogni membro deve installare Docker Desktop (Windows/Mac) o Docker Engine (Linux) seguendo la \href{https://docs.docker.com/get-docker/}{guida ufficiale}\textsubscript{Ultimo accesso: 2026/03/15}.
    \item \textbf{Avvio automatico:} Configurare Docker per l'avvio automatico all'accensione del sistema, se necessario.
\end{enumerate}

La configurazione dei Dev Containers avviene tramite i file \textbf{devcontainer.json} e \textbf{Dockerfile} presenti nella root del repository. I passi principali sono:

\begin{enumerate}
    \item \textbf{Scelta dell'immagine base:} Si seleziona un'immagine ufficiale oppure una specifica per il linguaggio/stack utilizzato (es. \textbf{node}, \textbf{python}, \textbf{texlive}).
    \item \textbf{Personalizzazione dell'immagine:} Nel \textbf{Dockerfile} si installano i tool necessari (es. \textbf{texlive-full}, \textbf{python3}, \textbf{git}, \textbf{aspell}, ecc.).
    \item \textbf{Configurazione delle estensioni VSCode:} Nel file \textbf{devcontainer.json} si elencano le estensioni consigliate/obbligatorie (es. \textbf{ms-vscode.cpptools}, \textbf{ms-azuretools.vscode-docker}, \textbf{James-Yu.latex-workshop}, \textbf{Atlassian.atlascode}, \textbf{GitHub.copilot}).
    \item \textbf{Configurazione delle variabili d'ambiente e mount di volumi:} Si possono definire variabili d'ambiente e montare directory locali utili allo sviluppo.
\end{enumerate}

Ogni configurazione è documentata e versionata nel repository, per ospitare tali configurazioni si deve utilizzare un branch dedicato "infrastruttura\_in\_lavorazione", dal quale poi effettuare un pull da ogni singolo branch. Questo evita di "sporcare" i branch di sviluppo con artefatti non pertinenti, provenienti dal main branch.
Questa procedura garantisce che tutti i membri lavorino in un ambiente identico, riducendo i problemi di compatibilità e semplificando l'onboarding.

\subparagraph{SonarQube ed estensioni correlate}
Per la configurazione di SonarQube è necessario:
\begin{enumerate}
    \item Installare SonarQube Server: Può essere installato localmente o utilizzato tramite un servizio cloud. Seguire la \href{https://docs.sonarcloud.io/}{guida ufficiale}\textsubscript{Ultimo accesso: 2026/03/15}.
    \item Configurare il progetto: Creare un nuovo progetto su SonarQube e ottenere il token di autenticazione.
    \item Integrazione con GitHub Actions: Aggiungere il token di SonarQube come secret nel repository GitHub e configurare il workflow di GitHub Actions per eseguire l'analisi del codice ad ogni commit, utilizzando il SonarQube Scanner.
    \item Visualizzazione dei report: I risultati dell'analisi saranno disponibili nel dashboard di SonarQube, con dettagli su code smells, vulnerabilità, e metriche di qualità del codice.
\end{enumerate}

Per l'integrazione nell'IDE VSCode è necessario installare l'estensione SonarQube, configurandola con l'URL del server fornito dall'amministratore. Il server fornirà un token di accesso che dovrà essere inserito nell'estensione per permettere l'autenticazione e il linting in tempo reale nel workspace.
