...
\begin{spacing}{1.1}
    \begin{longtable}{|c|c|p{3.2cm}|p{3.2cm}|p{5cm}|}
        ...
        \hline
        2.0.0 & 2026/05/01 & Georgescu Diana & - & Approvazione per baseline PB \\
        \hline
        1.3.2 & 2026/04/06 & Rocco Matteo A. & Soligo Lorenzo & Revisione e aggiunta Specifica Tecnica \\
        \hline
        1.3.1 & 2026/04/03 & Soligo Lorenzo & Gingillino Aaron & Revisione norme \\
        \hline
        1.3.0 & 2026/04/02 & Gingillino Aaron & Soligo Lorenzo & Aggiunta metriche di controllo \\
        \hline
        1.2.0 & 2026/04/01 & Vigolo Davide & Fattoni Antonio & Aggiornamento sezione di infrastruttura \\
        \hline
        1.1.0 & 2026/03/18    & Fattoni Antonio & Vigolo Davide & Aggiornamento sezione di codifica \\
        \hline
...
durante lo svolgimento delle attività di \glossario{progetto} volte allo
...
\href{https://7-zpus.github.io/Docs/assets/3_PB/DocumentiInterni/Glossario.pdf}{\ul{Glossario
        2.0}\setulcolor{black}}\ped{(v2.0.0)}.
\subsection{Riferimenti}
Il gruppo ha redatto il presente documento in conformità con lo Standard
ISO/IEC 12207:1995. In alcuni punti sono stati usati anche approfondimenti
tratti dalla versione ISO/IEC/IEEE 12207:2017. Questi riferimenti servono a
descrivere meglio gli approcci \glossario{\textit{Agile}} e iterativi che
...
Il fornitore deve estrarre i requisiti specificati dalla \glossario{proponente},
...
\begin{itemize}
    \item \textbf{Ruoli necessari}: identificare i ruoli necessari per lo sviluppo del progetto, come ad esempio sviluppatori, \glossario{verificatori}, analisti, ecc.
...
esigenze della proponente. È inoltre fondamentale identificare potenziali
rischi che possono insorgere nello svolgimento.

\paragraph{Procedura: pianificazione delle attività}
Il fornitore deve pianificare il lavoro a lungo termine. Deve definire \glossario{milestone} e obiettivi intermedi.
%qui come definire una milestone (guida concreta)
In particolare, l'identificazione delle milestone deve tenere conto dei seguenti aspetti:
\begin{itemize}
...
Il fornitore deve monitorare costantemente lo \glossario{stato di avanzamento
    lavori (SAL)}. Deve individuare eventuali deviazioni dal piano e adottare misure
correttive per mantenere il progetto sulla giusta traiettoria. Periodicamente,
tramite il cruscotto fornito da \glossario{Jira}, controlla quali attività sono state
completate, quali sono in corso e quali sono in ritardo. Se necessario,
rialloca le risorse liberate da \glossario{task} completate.
...
\begin{itemize}
    ...
    \item \textbf{Cruscotto}: per il monitoraggio delle \glossario{metriche} di processo e di prodotto, al fine di identificare eventuali criticità e adottare misure correttive tempestive.
\end{itemize}
...
realizzato, e dimostra la conformità ai requisiti attraverso \glossario{test di sistema}.
...
Il fornitore e la \glossario{committente} verificano che il lavoro svolto sia conforme ai
...
per l'utilizzo del prodotto. Per fare ciò si utilizza la funzione \textit{releases} di \glossario{GitHub}.
...
della proponente. Vengono mostrati assieme alla demo i risultati dei \glossario{test}, prova della
...
presentazione del team alle revisioni \glossario{RTB} e PB dove vengono esposti gli
...
Nel documento di
\href{https://7-zpus.github.io/Docs/assets/3_PB/DocumentiEsterni/AnalisiDeiRequisiti.pdf}{\ul{Analisi
...
(\glossario{Use Case}) e i requisiti (\textit{Requirements}) del
...
\begin{enumerate}
    ...
    \item \textbf{Definizione dei casi d'uso:} Rappresenta il nucleo del documento, con i casi d'uso principali descritti tramite \glossario{diagrammi UML} e schede tecniche che includono attori, \glossario{precondizioni}, \glossario{postcondizioni} e flussi principali.
    \item \textbf{Definizione dei requisiti:} Classificazione dettagliata dei requisiti individuati
\end{enumerate}
Il tracciamento dei requisiti è automatizzabile tramite \glossario{script} a partire dai requisiti definiti nel documento di \glossario{Analisi dei Requisiti}. \\
\textbf{Tipo}: Esterno \\
...
possono essere fonte di ambiguità nella comunicazioni tra i vari \glossario{stakeholder}
...
Il
\href{https://7-zpus.github.io/Docs/assets/3_PB/DocumentiEsterni/PianoDiProgetto.pdf}{\ul{Piano
...
completate in ciascuna \glossario{Sprint} e funge da guida per l'esecuzione del progetto. Il documento è diviso nelle seguenti sezioni:
\begin{enumerate}
    \item Introduzione
...
Il
\href{https://7-zpus.github.io/Docs/assets/3_PB/DocumentiEsterni/PianoDiQualifica.pdf}{\ul{Piano
        di Qualifica v2.0}\setulcolor{black}} \ped{(v2.0.0)} è il
...
metriche di valutazione vengono determinate dall'analisi dei
...
\paragraph{Specifica Tecnica}\label{specificaTecnica}
La \href{https://7-zpus.github.io/Docs/assets/3_PB/DocumentiEsterni/SpecificaTecnica.pdf}{\ul{Specifica Tecnica v1.0}\setulcolor{black}} \ped{(v1.0.0)} è il documento che descrive in modo rigoroso le scelte tecnologiche e l'architettura del prodotto da sviluppare. Fornisce un'analisi che parte dalla descrizione del contesto applicativo, per poi
approfondire la struttura dei container di sviluppo, delle componenti e del codice, al fine di
fornire una rappresentazione completa e coerente del sistema.
    ...
    \begin{adjustwidth}{-4cm}{-4cm}
        ...
        \begin{spacing}{1.1}
            \begin{tabular}{|c|c|}
                ...
                \hline
                Analisti                 & \begin{tabular}[c]{@{}p{10cm}@{}}Figura centrale nella fase di analisi del prodotto; identifica e documenta i requisiti del sistema garantendone chiarezza e completezza. È responsabile della redazione del documento \href{https://7-zpus.github.io/Docs/assets/3_PB/DocumentiEsterni/AnalisiDeiRequisiti.pdf}{Analisi dei Requisiti} \ped{(v1.0.0)}.\end{tabular}  \\
                \hline
                \glossario{Progettista}  & \begin{tabular}[c]{@{}p{10cm}@{}}Responsabile della progettazione dell'architettura del sistema e delle componenti software, assicurando che queste soddisfino i requisiti definiti nel documento di analisi. Redige il documento di \href{https://7-zpus.github.io/Docs/assets/3_PB/DocumentiEsterni/SpecificaTecnica.pdf}{Specifica Tecnica} \ped{(v1.0.0)}.\end{tabular}                                                                                                           \\
                \hline
...
dell'\glossario{architettura} del sistema, la codifica, i test e la \glossario{validazione}
del prodotto software.
\subsubsection{Attività: analisi dei requisiti}
L'Analisi dei Requisiti rappresenta una delle attività fondamentali
...
per le successive fasi di \glossario{progettazione} dell'architettura e di codifica,
supportando il lavoro dei progettisti e degli sviluppatori.

\paragraph{Procedura: raccolta e analisi dei requisiti}
Il fornitore deve individuare le esigenze dell'utente finale rispetto alle
funzionalità richieste del Software. Un passaggio chiave è pensare il mondo
\glossario{As is} e \glossario{To be}. In questo modo è più facile individuare
le funzionalità principali del software. Poi queste vengono approfondite con
l'aiuto e la comunicazione costante con la committente. Occorre anche
individuare vincoli e requisiti impliciti, derivanti dal dominio in cui opera
il software o non considerati dalla committente. Per farlo è utile richiedere
riferimenti sul dominio applicativo, come documentazione tecnica, legale o
normativa.
...
che rappresenta graficamente quanto scritto. È di fondamentale importanza
...
\begin{itemize}
    ...
    \item \textbf{Discord}: per la comunicazione sincrona tra i membri del gruppo
    \item \textbf{Gmail}: per la comunicazione asincrona con l'azienda proponente
    \item \textbf{VSCode}: con estensione con \glossario{IDE} di riferimento, utilizzato per la
...
Il fornitore sviluppa un Proof of Concept (PoC) per mostrare l'integrazione delle tecnologie scelte e la loro capacità di soddisfare i requisiti individuati.
Per questo artefatto si lascia ampio margine di manovra. Non è richiesto lo stesso livello di qualità del prodotto finale. La produzione ha forti limiti di risorse e l'obiettivo è solo dimostrare la fattibilità.

\paragraph{Strumenti}
\begin{itemize}
    \item \textbf{GitHub}: per la gestione del codice
          nella fase di fornitura
    \item \textbf{Jira}: per la suddivisione e il monitoraggio delle attività di progetto
          (\glossario{Issue Tracking System (ITS)})
    \item \textbf{Discord}: per la comunicazione sincrona tra i membri del gruppo
    \item \textbf{VSCode}: con estensione con \glossario{IDE} di riferimento, utilizzato per lo sviluppo del PoC (\glossario{Proof
              of Concept}).
\end{itemize}
...
L'attività di progettazione definisce l'architettura del sistema e le sue componenti. I requisiti individuati diventano una struttura software chiara e manutenibile. Il gruppo adotta il \textbf{modello C4} come riferimento per rappresentare l'architettura su più livelli. I diagrammi possono essere prodotti con \textbf{PlantUML} o \textbf{Draw.io}. In entrambi i casi, devono rispettare sintassi e convenzioni del modello C4.
...
Questo approccio consente una maggiore \glossario{efficienza}, distribuendo il carico di lavoro tra i membri 
...
\begin{itemize}
    ...
    \item \textbf{VSCode}: IDE principale, con estensione PlantUML per la visualizzazione in \glossario{anteprima} dei diagrammi durante la stesura.
...
\begin{itemize}
        \item Separare in modo chiaro responsabilità di \textbf{\glossario{front-end}}, \textbf{\glossario{back-end}} e servizi di integrazione, limitando l'accoppiamento tra componenti.
...
nello specifico, vengono definite norme di nomenclatura per variabili, funzioni, classi e file, che devono essere seguite da tutti i membri del team.
\begin{itemize}
    \item Utilizzo di \texttt{camelCase} per variabili e funzioni.
    \item Utilizzo di \texttt{PascalCase} per classi ed enum.
    \item Utilizzo di \texttt{UPPER\_SNAKE\_CASE} per costanti ed elementi di un enum.
    \item Ogni interfaccia deve essere preceduta dal prefisso \texttt{I} (es. \texttt{IUserService}). La rispettiva implementazione deve avere lo stesso nome senza prefisso (es. \texttt{UserService}).
    \item Ogni file deve contenere la definizione di al più una classe, nonchè possedere lo stesso nome della classe principale. Fanno eccezione i casi di classi utilizzate esclusivamente come supporto alla classe principale (es. classi di test, classi di eccezione, classi di utilità). In questo caso, il file deve essere nominato comunque con il nome della classe principale (es. \texttt{UserService.ts} può contenere la classe \texttt{UserService} e altre classi di supporto).
    \item Qualsiasi nome deve essere significativo e descrittivo, anche a discapito di nomi più brevi. Ad esempio, è preferibile utilizzare \texttt{getUserById} invece di \texttt{getUser}, in quanto il primo è più esplicativo riguardo alla funzionalità della funzione.
    \item Per le classi del main process, è consentita esclusivamente l'utilizzo di Dependency Injection tramite constructor injection con appositi decoratori. Non è consentita la risoluzione esplicita con chiamate al container DI.
\end{itemize}
...
\begin{itemize}
        \item evitare l'uso di variabili globali o pattern \textit{\glossario{singleton}} non adeguatamente giustificati;
...
\begin{itemize}
        ...
        \item \textbf{Sistema di versionamento}: per tracciare le modifiche, gestire \glossario{branch} e revisioni.
\end{itemize}
...
Vengono definite a seguire le linee guida da seguire per la definizione dei test:
\begin{itemize}
    \item I test devono essere progettati per essere \textbf{indipendenti} l'uno dall'altro, in modo da poter essere eseguiti in qualsiasi ordine senza influenzare i risultati.
    \item I test devono essere \textbf{ripetibili}, restituendo gli stessi risultati se eseguiti più volte con le stesse condizioni.
    \item I test devono essere \textbf{automatizzati} quando possibile, integrandoli nel processo di sviluppo per garantire feedback rapido e continuo.
    \item I test devono essere \textbf{documentati} in modo chiaro, specificando lo scopo del test, i dati di input, i risultati attesi e le condizioni di esecuzione. Questo avviene direttamente nel codice tramite \textbf{commenti} che decorano la definizione del test.
\end{itemize}
...
\end{itemize}
Maggior dettaglio sui livelli di test e le relative procedure di implementazione ed esecuzione è riportato nella sezione \ref{verifica}.
...
La pianificazione della documentazione avviene insieme alla pianificazione
delle attività di progetto. \\Durante ogni Sprint, il responsabile di progetto
assegna le attività ai membri del gruppo. Tiene conto delle competenze e della
disponibilità di ciascuno. Le scadenze vengono fissate per mantenere la
documentazione aggiornata e disponibile anche per eventuali attori esterni
(azienda proponente e committente). \\Per scrivere più facilmente i documenti
periodici (Verbali Interni, Verbali Esterni, Diario di Bordo), sono disponibili
...
\begin{itemize}
    \item Tutti i file dei documenti devono seguire la regola del \textbf{Pascal Case},
          ...
          in maiuscola.
    \item All'interno dei documenti, la data deve essere riportata nel \glossario{formato}
...
Lo stesso effettua il \glossario{merge} da "\textit{in\_lavorazione}" nel ramo principale
...
garantendo la tracciabilità delle modifiche e l'\glossario{integrità} dei rilasci.
...
Si identificano come CIs gli artefatti che compongono una baseline di progetto.
Nel caso di documenti ricorrenti come i verbali, si raggruppano più documenti omogenei in un unico CI (e.g. "Verbali").
Nel caso di codice si identificano le componenti principali, non ogni singola unità.
Questa scelta lascia al team una gestione più flessibile.
I CIs sono sempre associati a un \glossario{Epic} di Jira. In questo modo si garantiscono tracciabilità e gestione delle modifiche. In particolare, il processo di creazione deve seguire i seguenti passaggi:
\begin{enumerate}
...
corretto comportamento. I test devono essere
ripetibili e automatizzabili, isolati e il più efficienti possibili per garantire
efficienza. Devono essere inoltre consistenti, ovvero devono restituire sempre lo stesso risultato a parità di condizioni.\\
I test sono definibili tramite driver, ovvero un componente che simula il comportamento dell'entità "chiamante" verso le unità "chiamate". Lo stub invece costituisce un componente che simula il comportamento dell'entità "chiamata".
...
corrispondentemente alle aspettative definite per la singola task. È buona
...
\begin{itemize}
    ...
    \item \textbf{Aspell} per il controllo ortografico, utilizzato negli script automatizzati (\glossario{Github Action}).
    \item \textbf{Textstat} per il controllo di leggibilità, utilizzato negli script automatizzati (Github Action).
\end{itemize}

\subsubsection{Attività: verifica del codice}\label{verifica}
La verifica statica del codice avviene con SonarQube, che permette di individuare errori, vulnerabilità e code smell nel codice sorgente.
Il processo è integrato nel workflow di sviluppo. I controlli partono in automatico a ogni push o pull request.\\
Inoltre, il Team può consultare la dashboard di SonarQube per monitorare la qualità del codice a livello di branch e di progetto.\\
La verifica dinamica del codice, come già anticipato nella sezione \ref{test}, avviene in tre livelli, descritti in seguito.
...
Per avviare i test di unità è sufficiente eseguire il comando \texttt{npm run test} nella root del progetto, che avvierà l'esecuzione dei test frontend e backend. 

I test di unità possono essere di due tipi:
\begin{itemize}
    \item \textbf{Test funzionali}: verificano il codice secondo una logica "black-box", ovvero senza conoscenza della struttura implementativa interna. Solitamente utilizzati per verificare che l'unità rispetti la specifica funzionale. Per realizzare test esaustivi è necessario definirli secondo le "classi di equivalenza":
        \begin{itemize}
        \item \textbf{Valori ammissibili}: valori che rispettano le condizioni di validità specificate per l'unità.
        \item \textbf{Valori non ammissibili}: valori che violano le condizioni di validità specificate per l'unità.
        \item \textbf{Valori limite}: valori che si trovano ai confini delle condizioni di validità specificate per l'unità.
        \end{itemize}
    \item \textbf{Test strutturali}: verificano il codice secondo una logica "white-box", ovvero con conoscenza della struttura implementativa interna, sono detti anche "white box" tests.
...
I test di integrazione possono avvenire in due diverse direzioni:
\begin{itemize}
    \item \textbf{Top - down}: si verificano prima le componenti con maggior numero di dipendenti, verosimilmente dunque quelle di più alto livello, che dipendono da quelle di livello "più basso".
    \item \textbf{Bottom - up}: si verificano prima le componenti con minor numero di dipendenti, verosimilmente dunque quelle che sono la "base" su cui poggiano le altre componenti.
\end{itemize}

\paragraph{Procedura: test end-to-end di sistema}
I test end-to-end verificano il sistema nella sua interezza e simulano l'interazione dell'utente con l'interfaccia grafica. Controllano anche che tutte le componenti funzionino insieme.\\
Per facilitare l'esecuzione dei test, viene utilizzato il pattern Page Object. Questo permette di separare l'interazione con l'interfaccia in classi dedicate e rende i test più manutenibili.
Usando selettori stabili come \textit{data-testid}, non si dipende da etichette o parametri grafici.
\subparagraph{POM pattern per i test E2E}
Il pattern Page Object Model (POM) prevede classi dedicate per rappresentare le pagine dell'applicazione. 
Ogni classe contiene i metodi che l'utente può eseguire sulla pagina, come cliccare un pulsante o inserire testo in un campo.\\
Il flusso di interazione viene quindi descritto tramite questi metodi.\\
Inoltre, il POM centralizza la gestione dei selettori in un'unica classe.
\\ Per il tracciamento dei test di sistema è stato creato uno script che, a partire dai file di test e dai codici di test, genera un file con indicazioni sullo stato di copertura e sulle coordinate test.

\paragraph{Procedura: test di accettazione (Validazione)}
Sono test manuali che vengono eseguiti dalla committente per validare che ciò che è stato sviluppato sia in linea con le attese. Questo tipo di test è guidato dal team, e prevede sia l'organizzazione di un incontro che la messa a disposizione del software da testare. La domanda a cui risponde la validazione e dunque i test di accettazione è "Ho sviluppato il software corretto?", rispetto al ruolo della verifica che invece risponde alla domanda "Ho sviluppato correttamente il software?".

\paragraph{Nomenclatura}
Viene riportata in seguito la nomenclatura adottata per i test:
\begin{center}
    \textbf{T[tipo test]-[progressivo numerico]}
\end{center}

\textbf{Tipo di test:}
\begin{itemize}
    \item \textbf{U}: \textbf{U}nità
    \item \textbf{I}: \textbf{I}ntegrazione
    \item \textbf{S}: \textbf{S}ystem
    \item \textbf{A}: \textbf{A}ccettazione
\end{itemize}

lo stato di un test, descritto nel documento: \href{https://7-zpus.github.io/Docs/assets/3_PB/DocumentiEsterni/PianoDiQualifica.pdf}{Piano di Qualifica\textsubscript{(v2.0.0)}}, segue la seguente specifica:
\begin{itemize}
    \item \textbf{NI}: \textbf{N}on \textbf{I}mplementato
    \item \textbf{I}: \textbf{I}mplementato
    \item \textbf{S}: \textbf{S}uperato
\end{itemize}

\paragraph{Strumenti}
\begin{itemize}
    \item \textbf{SonarQube:} per l'analisi statica del codice, integrato nel processo di sviluppo.
...
La validazione è un processo soggettivo e orientato all'utente. Verifica che il prodotto sviluppato
soddisfi i requisiti della proponente. Valuta l'adeguatezza del prodotto. Serve a capire
se il software risponde ai bisogni dell'utente.
...
passaggi:
\begin{enumerate}
    \item Identificazione dei requisiti utente chiave da validare, in collaborazione con la proponente.
    \item Definizione dei criteri di accettazione e dei risultati attesi.
    \item Assegnazione delle responsabilità per la progettazione e l'esecuzione dei test.
\end{enumerate}
...
gruppo, finalizzata a rivedere insieme sviluppo e documentazione. Questa attività è utile quando si raggiunge una milestone. Gli oggetti della revisione congiunta coincidono spesso con dei Configuration Items. Il processo prevede la partecipazione di un sottoinsieme dei membri del gruppo, scelti in base al loro coinvolgimento nelle attività che hanno contribuito alle modifiche del Configuration Item.
...
Il processo di \glossario{garanzia della qualità} applica in modo sistematico le metriche definite. Coordina anche garanzia della qualità, verifica, validazione e miglioramento. L'obiettivo è mantenere alta la qualità del prodotto finale e dei processi. \\Questo processo segue tutte le fasi del ciclo di vita del progetto.
...
\begin{enumerate}
    \item \textbf{Definizione delle metriche di processo}: vengono scelte le metriche chiave per ogni processo. Si privilegiano metriche facili da raccogliere e utili da misurare.
    \item \textbf{Definizione delle metriche di prodotto}: vengono scelte le metriche chiave per il prodotto finale. Devono restare pertinenti al dominio applicativo.
    \item \textbf{Definizione delle soglie di accettabilità}: per ogni metrica si fissano soglie minime realistiche e raggiungibili.
    \item \textbf{Definizione dei valori di ottimalità}: per ogni metrica si fissano anche i valori ideali da raggiungere.
\end{enumerate}
...
\paragraph{Strumenti}
\begin{itemize}
    \item \textbf{Jira}: Per la gestione delle attività di pianificazione della qualità e il tracciamento degli obiettivi.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Controllo automatizzato:}
          \begin{itemize}
              \item Le GitHub Actions controllano ogni commit.
          \end{itemize}
    \item \textbf{Controllo manuale:}
          \begin{itemize}
              \item I verificatori controllano codice e documenti.
              \item Quando serve, fanno test manuali.
          \end{itemize}
    \item \textbf{Utilizzo del Cruscotto di Qualità:}
          \begin{itemize}
              \item Le metriche sono visibili nel Piano.
              \item Si monitorano trend e scostamenti.
          \end{itemize}
\end{enumerate}

\paragraph{Strumenti}
\begin{itemize}
    \item \textbf{Github Actions}: Per l'automazione del monitoraggio delle metriche di qualità.
    \item \textbf{Jira}: Per il tracciamento delle metriche di processo.
    \item \textbf{Strumenti di analisi statica}: Per il calcolo automatico di metriche di codice (complessità, coverage)
    \item \textbf{Piano di Qualifica}: valutazione delle metriche tramite il cruscotto di qualità.
\end{itemize}
...
\begin{itemize}
    ...
    \item Si valuta l'\glossario{efficacia} delle azioni correttive implementate
...
\paragraph{Strumenti}
\begin{itemize}
    \item \textbf{Jira}: per la raccolta e l'analisi delle metriche di processo.
    \item \textbf{Strumenti di analisi statica}: Per il calcolo automatico di metriche di codice (complessità, coverage)
    \item \textbf{Piano di Qualifica}: per la valutazione delle metriche
\end{itemize}
...
\paragraph{Strumenti}\label{strumenti_gestione_qualita}
\begin{itemize}
    ...
    \item \textbf{Jira} Per il monitoraggio in tempo reale dello stato delle metriche,
          il tracciamento delle non conformità, le azioni correttive e la condivisione di \glossario{report} di qualità con la proponente
\end{itemize}
...
all'\glossario{amministratore} tutte le informazioni necessarie per inserirle in Jira. Le
informazioni sono:
\begin{itemize}
...
\subparagraph{Git}
Per la configurazione di \glossario{Git} è necessario che ogni membro del team configuri il proprio ambiente locale, impostando nome e email con i comandi appositi.
...
\metricdef{MPC-6}{Estimate to Complete}
{Rappresenta l'ammontare rimanente stimato per completare il progetto}
...
\metricdef{MPC-8}{Deployment Frequency}
{Frequenza con cui il team rilascia nuove versioni del software, potenzialmente trasportabili in produzione.}
\metricformula{DF = \frac{\text{Numero di deployment}}{\text{Periodo di tempo}}}

\metricdef{MPC-9}{Requirements Stability Index}
{Rappresenta la stabilità dei requisiti del prodotto software nel tempo.}
...
\metricdef{MPC-10}{Average Build Time}
{Tempo medio impiegato per completare il processo di build e test automatici.}
\metricformula{T_{avg} = \frac{\sum_{i=1}^{n} T_{build\_i}}{n}}

\metricdef{MPC-11}{Sprint Velocity}
{Quantità di lavoro in uno Sprint, considerando le aggiunte, gli elementi non completati, e le modifiche al backlog.}
\metricformula{SV = \#\text{Workitem completati} - (\#\text{Workitem aggiunti} + \#\text{Workitem non completati})}

\metricdef{MPC-12}{Burndown Adherence}
{Rapporto tra il coefficiente di elementi completati e il coefficiente ideale. Questo indice ideale va dal numero di workitem nel momento dell'apertura dello Sprint a 0 al termine dello Sprint.}
\metricformula{BA = \frac{\frac{d \text{Consumo reale}}{dx}}{\frac{d \text{Consumo ideale}}{dx}}}

\paragraph{Processo di Documentazione}

\metricdef{MPC-13}{Indice di Gulpease}
{Indice che valuta la leggibilità di un testo in lingua italiana. Valori bassi indicano bassa leggibilità.}
\metricformula{G = 89 + \frac{300 \times (\#\text{Frasi}) - 10 \times (\#\text{Lettere})}{\#\text{Parole}}}

\metricdef{MPC-14}{Correttezza Ortografica}
{Misura la densità di errori ortografici presenti nella documentazione.}
\metricformula{C = \#\text{errori ortografici}}

\paragraph{Processo di Verifica}

\metricdef{MPC-15}{Code review turnaround time}
{Tempo medio impiegato per completare una revisione del codice in vista di una Pull Request.}
\metricformula{CRT = \frac{\sum_{i=1}^{n} T_{review\_i}}{n}} $T_{review\_i}$ tempo impiegato per completare la revisione della PR $i$.

\metricdef{MPC-16}{Test success rate}
{Percentuale di test superati rispetto al totale dei test eseguiti.}
\metricformula{TSR = \left( \frac{\#\text{Test superati}}{\#\text{Test eseguiti}} \right) \times 100}

\metricdef{MPC-17}{Code Coverage}
{Percentuale di codice coperta da test}
...
\metricdef{MPC-18}{Rischi non previsti}
{Numero di rischi non previsti ad inizio periodo, per ciascun periodo di progetto.}
\metricformula{RNP_i = \#\text{Rischi non previsti nel periodo } i}

\paragraph{Processo di Gestione della qualità}

\metricdef{MPC-19}{Metriche soddisfatte}
{Percentuale di metriche di qualità che vengono soddisfatte in un determinato istante di tempo.}
...
\metricdef{MPD-1}{\glossario{Requisiti obbligatori} soddisfatti}
{Percentuale di requisiti funzionali obbligatori implementati e testati.}
...
\metricdef{MPD-2}{\glossario{Requisiti opzionali} soddisfatti}
{Percentuale di requisiti funzionali opzionali implementati e testati.}
...
\metricdef{MPD-3}{\glossario{Requisiti desiderabili} soddisfatti}
{Percentuale di requisiti funzionali desiderabili implementati e testati.}
...
\metricdef{MPD-4}{Broken links}
{Numero di riferimenti ipertestuali all'interno dell'applicazione che non portano alla risorsa corretta o non portano ad alcuna risorsa.}
\metricformula{BL = \#\text{Broken Links}}

\metricdef{MPD-5}{Branch Coverage}
{Percentuale di rami decisionali (if, switch, loop) percorsi durante i test.}
\metricformula{BC = \left( \frac{\text{Rami testati}}{\text{Rami totali}} \right) \times 100}

\metricdef{MPD-6}{Statement Coverage}
{Percentuale di istruzioni (statements) del codice sorgente eseguite durante i test automatici.}
\metricformula{SC = \left( \frac{\text{Istruzioni testate}}{\text{Istruzioni totali}} \right) \times 100}

\subsubsection{Usabilità}

\metricdef{MPD-7}{Profondità di Navigazione}
{Numero massimo di click necessari per raggiungere una qualsiasi pagina di contenuto partendo dalla vista principale.}
...
\metricdef{MPD-8}{Indexing time}
{Tempo di \glossario{indicizzazione} per un \glossario{DiP} di dimensioni standard (1-4GB).}
\metricformula{IT = \text{Tempo di indicizzazione (secondi)}}

\metricdef{MPD-9}{Search Time}
{Tempo medio di risposta per un set di \glossario{query} predefinite.}
...
\metricdef{MPD-10}{Average CPU usage}
{Percentuale media di utilizzo della CPU durante l'esecuzione del software. Si considera un campione di misurazioni effettuate in condizioni di carico (durante l'indicizzazione o la ricerca) nonché in idle. Quest'ultimo per assicurare che il software non consumi risorse inutilmente quando non è in uso.}
\metricformula{CPU_{avg} = \sum_{i=1}^{n} \frac{\text{Utilizzo CPU}_i}{n} \quad (\text{dove }n = \text{numero di misurazioni})}

\metricdef{MPD-11}{Peak memory usage}
{Massima quantità di memoria RAM utilizzata durante l'esecuzione del software. Permette di individuare eventuale scarsa gestione delle risorse.}
\metricformula{MEM_{peak} = \max(\text{Utilizzo RAM}_i) \quad (\text{dove }i = 1, \dots, n, \text{istante di tempo } i)}

\subsubsection{Manutenibilità}

\metricdef{MPD-12}{Complessità Ciclomatica}
{Misura la complessità del flusso di controllo del codice. Valori superiori a 15 indicano codice difficile da testare e manutenere.}
\metricformula{M = E - N + 2P \quad (\text{dove } E=\text{archi}, N=\text{nodi}, P=\text{componenti connessi})}

\metricdef{MPD-13}{Accoppiamento tra Classi (CBO)}
{Numero di classi a cui una determinata classe è accoppiata (usa o è usata da). Un valore alto indica scarsa modularità.}
\metricformula{CBO = \frac{\#\text{Dipendenze}}{\#\text{Componenti}}}

\metricdef{MPD-14}{Code smells}
{Numero di code smells, misurato ogni 1000 righe di codice (KLOC). I code smells sono indicatori di potenziali problemi di design o implementazione.}

\subsubsection{Portabilità}

\metricdef{MPD-15}{SO Compatibility}
{Numero di sistemi operativi supportati dal software. Un valore alto indica una maggiore portabilità.}
...
\paragraph{SonarQube}
Piattaforma centrale per la \glossario{Continuous Inspection} del codice
