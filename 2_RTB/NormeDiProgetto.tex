...
\begin{itemize}
    \item \textbf{Pianificazione della documentazione}: definizione delle linee guida per la stesura dei documenti, inclusi formati, modelli e standard di qualità e assegnazione della redazione ai membri del gruppo. Più in \ref{pianificazioneDocs}
...
di VSCode \ref{vscode}. Grazie a quest'ultima è possibile creare direttamente il
...
\end{itemize}
A questo punto il responsabile effettua il merge della feature nel ramo principale (\textit{verbali in lavorazione, norme in lavorazione, etc}), base per la versione ufficiale di rilascio corrispondente alla \glossario{milestone}.
...
la divisione dei compiti. Tali procedure sono suddivise per ruolo:
\begin{itemize}
    ...
    \item Completata la revisione, il revisore può:
          \begin{itemize}
              \item Approvare la PR, notificando all'autore l'approvazione. A sua volta l'autore
                    ...
                    documenti pronti all'approvazione finale del responsabile. La procedura per la
...
La necessità di tracciamento delle attività di progetto richiede l'adozione capillare, costante e frequente degli Smart Commit, al fine di un aggiornamento tempestivo e che rispecchi il reale avanzamento delle task. \\
Gli Smart Commit sono una funzionalità di Jira che permette di collegare i commit di GitHub alle issue di Jira tramite messaggi speciali nei commit. Questo consente di automatizzare la gestione delle issue, come la chiusura automatica o l'aggiornamento dello stato, direttamente dal messaggio di commit. \\ Lo Smart Commit segue la seguente sintassi:
\begin{verbatim}
