...
\begin{table}[H]
    \begin{adjustwidth}{-4cm}{-4cm}
        ...
        \begin{tabular}{|c|c|c|c|c|}
            ...
            0.9 & 6/2/2026 & Laoud Zakaria & Soligo Lorenzo & Stesura paragrafi 3.6, 3.7, 3.8 \\
            \hline
...
\paragraph{Pianificazione delle revisioni}
Come concordato con la proponente (§2.1), con cadenza bisettimanale il gruppo di lavoro si incontra con l’azienda tramite Google Meet per discutere lo stato di avanzamento 
del progetto.\\
L’amministratore si occupa della creazione della \glossario{task} Jira relativa all’incontro, assegnando anche la scrittura e verifica del verbale.

\paragraph{Preparazione dei materiali}
...
\paragraph{Conduzione della revisione}
...
\begin{enumerate}
    ...
    \item Il verbale approvato viene archiviato nel repository secondo le procedure di documentazione (§3.1.2)
\end{enumerate}
...
\begin{table}[H]
    \begin{adjustwidth}{-4cm}{-4cm}
        ...
        \begin{tabular}{|c|c|}
            ...
            \hline
            Amministratore & \begin{tabular}[c]{@{}c@{}} Gestisce i problemi legati all'infrastruttura e agli strumenti, \\ come descritto in §4.1 \end{tabular} \\
            \hline
...
\paragraph{Identificazione del problema}
...
\paragraph{Analisi e classificazione}
...
\paragraph{Sviluppo della soluzione}
...
\paragraph{Implementazione e verifica}
La soluzione viene implementata seguendo le normali procedure di sviluppo:
\begin{enumerate}
    \item Creazione del branch di lavoro secondo gli standard (§3.2.2)
    \item Implementazione della soluzione con commit e smart commit (§3)
    ...
    \item Merge della soluzione secondo le procedure di revisione (§3.1.3)
\end{enumerate}

\subsubsection{Procedure di risoluzione}
...
\subsubsection{Integrazione con altri processi}
Il processo di risoluzione dei problemi si integra con:
\begin{itemize}
    \item \textbf{Gestione dei rischi operativi (§2.3.2):} I problemi ricorrenti vengono aggiunti al registro dei rischi
    \item \textbf{Analisi degli incidenti (§5.1.1):} Per problemi infrastrutturali seguono le procedure specifiche
    \item \textbf{Processo di miglioramento (§5.1):} I problemi sistemici alimentano i cicli di miglioramento continuo
...
\paragraph{Pianificazione della qualità}
...
\begin{enumerate}
    ...
    \item \textbf{Selezione delle metriche applicabili:} Tra quelle definite nella sezione §6, vengono scelte le metriche più rilevanti per lo sprint corrente
...
\paragraph{Monitoraggio continuo}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item La Dashboard Jira personalizzata (§5) fornisce una vista in tempo reale dello stato delle metriche
        \item Vengono monitorati trend e deviazioni rispetto agli obiettivi
    \end{itemize}
\end{enumerate}

\paragraph{Valutazione periodica}
...
\begin{enumerate}
    ...
    \item \textbf{Analisi della causa:} Si applica il processo di risoluzione dei problemi (§3.7) per identificare le cause radice
...
\subsubsection{Procedure di gestione della qualità}
...
\begin{itemize}
    \item \textbf{Processo di garanzia della qualità (§3.3):} Fornisce il framework metodologico per le attività di quality assurance
    \item \textbf{Processo di verifica (§3.4):} Esegue le verifiche tecniche secondo le procedure definite
    \item \textbf{Processo di validazione (§3.5):} Conferma che il prodotto soddisfi le esigenze degli utenti
    \item \textbf{Processo di revisione congiunta (§3.6):} Comunica alla proponente lo stato della qualità e raccoglie feedback
    \item \textbf{Processo di risoluzione dei problemi (§3.7):} Gestisce le non conformità rispetto agli standard di qualità
    \item \textbf{Processo di miglioramento (§4.2):} Utilizza i dati di qualità per identificare aree di miglioramento
\end{itemize}

\subsubsection{Metriche di gestione della qualità}
...
Queste meta-metriche vengono rendicontate nel Piano di Qualifica insieme alle metriche di processo e prodotto definite nella sezione §6.

\subsubsection{Strumenti}
\begin{itemize}
    \item \textbf{Piano di Qualifica:} Documento di riferimento per obiettivi, metriche e soglie di qualità
    \item \textbf{Jira e Dashboard Jira (§5):} Per il monitoraggio in tempo reale dello stato delle metriche, 
    il tracciamento delle non conformità, le azioni correttive e la condivisione di report di qualità con la proponente
    \item \textbf{GitHub Actions (§5):} Per l'automazione delle verifiche di qualità (Gulpease, build, test)
