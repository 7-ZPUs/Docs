    ...
    \begin{adjustwidth}{-4cm}{-4cm}
        ...
        \begin{spacing}{1.1}
            \begin{tabular}{|c|c|}
                ...
                \textbf{Ruolo}                                      & \textbf{Specifica}                                                                                                                                                                                                                  \\
                \hline
                Responsabile di progetto                            & \begin{tabular}[c]{@{}p{10cm}@{}}Coordinatore dell'attività di revisione congiunta. Regola il ritmo e la struttura della sessione di revisione. Si assicura che i materiali di discussione siano pronti e accessibili.\end{tabular} \\
                \hline
...
L'analisi e la classificazione sistematica dei problemi permettono di stabilire
priorità di intervento, allocare risorse in modo efficiente e prevenire che
criticità minori si trasformino in blocchi significativi per il progetto.
...
Una volta completata l'analisi e la classificazione del problema, il team
procede con lo sviluppo di una soluzione strutturata. Questa fase richiede un
approccio metodico che coinvolga i membri più competenti nella ricerca di
alternative e nella valutazione delle possibili conseguenze.
...
Una volta completata l'analisi e la classificazione del problema, il team
procede con l'implementazione sistematica della soluzione identificata. Questa
fase è critica poiché trasforma l'analisi teorica in azioni concrete che
risolvono il problema alla radice. L'implementazione deve essere accompagnata
da una verifica rigorosa per assicurare che la soluzione sia efficace, non
introduca nuove problematiche e sia conforme agli standard di qualità del
progetto. Solo attraverso una verifica accurata è possibile garantire che il
problema sia effettivamente risolto e che il sistema rimanga stabile e
affidabile.
...
\subsubsection{Ruoli}
Vengono qui raccolti per praticità i ruoli coinvolti del processo di
risoluzione dei problemi, con una breve descrizione delle responsabilità di
ciascuno.
\begin{table}[H]
    \begin{adjustwidth}{-4cm}{-4cm}
        \centering
...
Per ogni milestone, il responsabile coordina la pianificazione degli obiettivi
di qualità:
\begin{enumerate}
...
identificare eventuali deviazioni dagli standard di qualità imposti. Pe
l'analisi si applica il processo di risoluzione dei problemi
(\ref{proc_risoluzione_problemi}) in modo da identificare le cause radice
...
al grado di importanza, complessità ed estensione delle deviazioni delle pre.
\begin{itemize}
...
\subsubsection{Attività: Implementazione (Criteri di Scelta)}\label{ImplementazioneInfrastruttura}
...
\subsubsection{Attività: Creazione e Configurazione}\label{creazioneInfrastruttura}
Sulla base dei criteri descritti, il Team ha configurato e documentato la seguente infrastruttura operativa:
...
\begin{itemize}
    \item \textbf{Giustificazione (Costi ed Equipment):} IDE gratuito, leggero e standard de facto, garantisce che tutti i membri operino in un ambiente omogeneo indipendentemente dal proprio sistema operativo.
...
\subsubsection{Attività: Manutenzione dell'Infrastruttura}
L'infrastruttura non è statica ma viene manutenuta, monitorata e aggiornata per soddisfare le esigenze delle diverse fasi di progetto.

\paragraph{Monitoraggio degli Strumenti}
Il Team raccoglie costantemente feedback sull'efficacia degli strumenti (tramite Discord e le retrospettive su Jira). Se vengono identificate inefficienze (es. colli di bottiglia nelle automazioni), il Responsabile valuta un intervento correttivo.

\paragraph{Modifiche e Ampliamenti Pianificati}
In vista dell'ingresso nella fase di produzione del codice, l'infrastruttura verrà modificata seguendo le medesime procedure di selezione iniziali. Sono già pianificati i seguenti ampliamenti:
\begin{itemize}
