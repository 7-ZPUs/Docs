...
previste. Ad ogni documento principale è legata una branch
\texttt{in\_lavorazione} che mantiene le versioni aggiornate pronte alla
...
Il processo di revisione congiunta è un'attività sincrona tra i membri del gruppo, finalizzata al terminare un'insieme di sviluppo o di documentazione coese tra loro.
Questa attività risulta particolarmente utile se eseguita in concomitanza del raggiungimento di una milestone. Verosimilmente, oggetti della revisione congiunta coincideranno
con dei Configuration Items. Il processo di revisione congiunta prevede la partecipazione di un sottoinsieme dei membri del gruppo, selezionati in base al loro coinvolgimento
nelle attività che hanno contribuito alle modifiche del suddetto Configuration Item.

\subsubsection{Attività: Pianificazione}
La pianificazione della revisione congiunta è stabilita dal responsabile in collaborazione con i membri del team che si sono occupati dello sviluppo del Configuration Item. L'attività viene pianificata secondo quanto scritto in §\ref{gestione_processi_organizzativi}.

\paragraph{Procedura: Identificazione dei partecipanti}
Il responsabile identifica i membri del team che parteciperanno alla revisione congiunta, tenendo conto dei seguenti criteri:
\begin{enumerate}
...
Il responsabile, in collaborazione con il team, identifica e documenta il materiale da sottoporre a revisione congiunta:
\begin{enumerate}
...
Il responsabile pianifica la sessione di revisione congiunta seguendo i seguenti passaggi:
\begin{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{GitHub}: Per l'accesso rapido al materiale di revisione (documenti, codice, branch).
\end{itemize}
...
La fase iniziale della revisione prevede una piccola pianificazione dell'ordine di revisione del materiale, che tenga conto della complessità e delle relazioni tra le varie parti che lo compongono.
Si lascia la libertà a chi conduce la revisione di stabilire l'ordine e la logica più adatta al caso specifico, tenendo conto dei seguenti criteri:
\begin{enumerate}
...
\paragraph{Procedur: Revisione del materiale}
Durante la revisione ai partecipanti esaminano attentamente il materiale, discutendo eventuali dubbi, criticità o proposte di miglioramento.
Ogni problema emerso durante la revisione viene comunicato al responsabile, che si occupa poi di pianificare le attività di risoluzione assieme ai revisori.
...
Una identificazione precoce e accurata dei problemi consente al gruppo di
intervenire proattivamente, minimizzando gli impatti sulle attività di progetto
e prevenendo l'accumularsi di criticità che potrebbero compromettere la qualità
del prodotto finale o il rispetto delle scadenze pianificate.
...
L'analisi e la classificazione sistematica dei problemi permettono di stabilire priorità di
intervento, allocare risorse in modo efficiente e prevenire che criticità
minori si trasformino in blocchi significativi per il progetto.
...
Una volta completata l'analisi e la classificazione del problema, il team procede con lo sviluppo di una soluzione strutturata. 
Questa fase richiede un approccio metodico che coinvolga i membri più competenti nella ricerca di alternative e nella valutazione delle possibili conseguenze.
...
Una volta completata l'analisi e la classificazione del problema, il team procede con l'implementazione sistematica della soluzione identificata. 
Questa fase è critica poiché trasforma l'analisi teorica in azioni concrete che risolvono il problema alla radice. 
L'implementazione deve essere accompagnata da una verifica rigorosa per assicurare che la soluzione sia efficace, non introduca nuove problematiche e sia conforme agli standard di qualità del progetto. 
Solo attraverso una verifica accurata è possibile garantire che il problema sia effettivamente risolto e che il sistema rimanga stabile e affidabile.
...
\subsection{Processo di gestione della qualità}\label{gestione_qualita}
...
Per ogni milestone, il responsabile coordina la pianificazione degli
obiettivi di qualità:
\begin{enumerate}
    \item \textbf{Definizione degli obiettivi della milestone:} In base alle attività pianificate, vengono identificati gli obiettivi specifici di qualità da raggiungere
    \item \textbf{Selezione delle metriche applicabili:} Tra quelle definite nella sezione \ref{metriche_qualita}, vengono scelte le metriche più rilevanti per le aspettative della milestone
...
La valutazione periodica al fine del miglioramento continuo. Esistono però diverse forme di valutazione periodica.
\paragraph{Procedura: Review settimanale}
 Durante le riunioni di team, vengono discusse brevemente le metriche principali e le eventuali criticità emerse. Questa review è scaturita dalla analisi del cruscotto e delle sue metriche. 
...
Attraverso il monitoraggio continuo o le review periodiche è possibile identificare eventuali deviazioni dagli standard di qualità imposti.
Pe l'analisi si applica il processo di risoluzione dei problemi (\ref{proc_risoluzione_problemi}) in modo da identificare le cause radice
\paragraph{Procedura: Pianificazione dell'intervento}
La pianificazione del piano di intervento correttivo si differenzia in in base al grado di importanza, complessità ed estensione delle deviazioni delle pre.
          \begin{itemize}
...
Le seguenti linee guida orientano il Team nella gestione sistematica della qualità:
\begin{enumerate}
...
Per una gestione efficiente delle attività di progetto è necessario che la
scelta degli strumenti infrastrutturali a sostegno dei processi sia ben
ponderata. L'attività di implementazione di occupa di questo aspetto.

\subsubsection{Procedura: Analisi degli strumenti}
Uno strumento per essere incluso tra gli strumenti del team deve prima di tutto
rispondere ai requisiti del processo nel quale viene implementato. È necessario
poi considerare una serie di variabili che non possono essere tralasciati. Tra
questi:
\begin{itemize}
...
Una volta che è stato scelto uno strumento è necessario che venga aggiornato il
documento delle norme con l'inserimento all'interno del processo interessato
del riferimento alle procedure di utilizzo ddel nuovo strumento. \\Ci si
aspetta inoltre che maggiori informazioni siano disponibili alla sezione
specifica dello strumento nella \textit{Knowladge Base} del team
(\ref{knowladge_base})
...
La realizzazione dell'infrastruttura è tale solo se gli strumenti e le loro
procedure vengono applicati dai membri del team in modo coerente con quanto
descritto dalle Norme di Progetto.
\paragraph{Procedura: Studio e allineamento}
Per favorire un uso coerente degli strumenti opportuno che ogni membro del team
rimanga aggiornato sull'evoluzione dell'infrastruttura. L'allineamento parte
dallo studio personale delle spiegazioni delle Norme di Progetto e sfocia
nell'applicazione sul campo. \\Questo non esclude la possibilità in organizzare
incontri sincroni di aggiornamento per chiarimenti aggiuntivi.
\paragraph{Strumenti}
...
\paragraph{Procedura: Monitoraggio}
Viene effettuato un monitoraggio costante delle prestazioni degli strumenti per
identificare eventuali problemi o inefficienze. Tale monitoraggio è più
efficacie se non è limitato al solo ruolo del amminsitratore. Tutti i membri
del team dovrebbero segnalare funzionamenti non ottimali all'amministratore in
modo che quest'ultimo possa indagare sulle origini di questi malfunzionamenti e
in caso valutare la modifica dell'infrastruttura.
\paragraph{Procedura: Segnalazione}
Nel caso in cui ci sia da investigare un funzionamento anomalo, è bene che in
seguito ad una pronta comunicazione tramite i canali sincroni del gruppo,
l'amministratore crei un task Jira. \\Una volta fatto ciò viene analizzato il
malfunzionamento e nel caso sia un evento non ignorabile come "caso isolato",
viene riaperta la ricerca di nuovi strumenti che sostituiscano quello corrente
con le stesse modalità descritte qui \ref{ImplementazioneInfrastruttura}
...
In vista dell'attività di produzione di codice, l'infrastruttura verrà
aggiornata e ampliata per poter rimanere al passo con le nuove necessità. Sono
in particolare previste le seguenti aggiunte:
\begin{itemize}
...
I materiali e le note della formazione vengono salvati nelle Norme di Progetto
per rendere disponibili a tutti i materiali di Formazione.
...
Nei giorni successivi alla formazione, vengono assegnati esercizi pratici o
