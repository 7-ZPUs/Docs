...
\begin{enumerate}
    ...
    \item \textbf{Definizione degli obiettivi:}  chiari e misurabili per la fornitura del prodotto software, in modo da garantire un allineamento con le aspettative del committente. Per esempio diversi membri del gruppo hanno espresso l'obiettivo della laurea a luglio 2026, perciò è necessario pianificare le attività di progetto in modo da rispettare questa scadenza.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Definizione del contenuto della proposta:}  includendo una descrizione dettagliata del prodotto software proposto, le funzionalità chiave, i benefici attesi e come il prodotto soddisferà i requisiti del committente.
    \item \textbf{Stima dei costi e pianificazione delle attività:} per lo sviluppo del prodotto software, tenendo conto delle risorse necessarie, del tempo stimato per completare le attività e di eventuali costi aggiuntivi. Questa stima si concretizza nella sezione di preventivo di costi del documento di Piano di Progetto\ref{pianoDiProgetto}.
    \item \textbf{Identificazione e analisi dei requisiti:} che garantisca la più completa comprensione delle esigenze del committente. Questa attività si concretizza nel documento di Analisi dei Requisiti\ref{analisiDeiRequisiti}.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Definizione del modello di ciclo di vita del prodotto:}  selezionando un approccio di sviluppo (ad esempio, Agile, Waterfall, DevOps) che sia più adatto alla complessità del progetto e ai rischi associati.
...
\end{enumerate}
Le particolarità di pianificazione del progetto sono riportate nei rispettivo processo. 
...
\subsubsection{Attività: Verifica e validazione}
...
\paragraph{Procedure di verifica e validazione}
Il fornitore deve seguire una serie di procedure per verificare e validare correttamente lo sviluppo del prodotto:
\begin{enumerate}
    \item \textbf{Definizione delle modalità di rendicontazione:}  concordando con la proponente le modalità e i tempi di comunicazione dello stato di avanzamento del progetto, inclusi report periodici, incontri di aggiornamento e strumenti di monitoraggio.
    \item \textbf{Rendicontazione dello stato di avanzamento:} fornendo agli \glossario{stakeholder} documenti dettagliati che dimostrino la verifica e validazione dei processi, come report di test, risultati delle verifiche di qualità e aggiornamenti sullo sviluppo del prodotto.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Esposizione delle funzionalità:} presentando al proponente le funzionalità del prodotto, illustrando come soddisfa i requisiti e rispondendo a eventuali domande o dubbi da parte del proponente.
...
Il documento di \href{https://cdn.jsdelivr.net/gh/7-zpus/Docs@main/1_Candidatura/LetteraDiPresentazione.pdf}{\ul{Lettera di presentazione}\setulcolor{black}} \ped{(ultimo accesso: 17/11/2025)} formalizza la candidatura del team ai committenti per lo sviluppo del progetto proposto, dichiarando la data di consegna, il budget stimato e la composizione ufficiale del team.

\paragraph{Verbali interni ed esterni}
...
\paragraph{Lettera di candidatura}
...
\end{enumerate}
Il tracciamento dei requisiti è automatico è automatizzabile tramite script a partire dai requisiti definiti nel documento di Analisi dei Requisiti.
...
\\ È automatizzato anche l'aggiornamento del glossario nella pagina web, per una maggiore leggibilità.
...
La lettera di presentazione è il documento necessario alla candidatura per la milestone di revisione di avanzamento RTB (\textit{Requirements and Technology Baseline}). Essa contiene le informazioni sul repository di progetto, il puntatore al \glossario{Proof of Concept (PoC)}, il consuntivo di spesa e preventivo a finire del progetto.
...
\begin{table}[H]
    \begin{adjustwidth}{-4cm}{-4cm}
        ...
        \begin{tabular}{|c|c|}
            ...
            \hline
            Responsabile di progetto & \begin{tabular}[c]{@{}c@{}} Coordina le attività del team,\\ garantisce il rispetto delle scadenze e \\mantiene aggiornato il Way of Working. \end{tabular} \\
            \hline
            Amministratore & \begin{tabular}[c]{@{}c@{}} Si occupa del controllo e mantenimento \\dell'infrastruttura, assicurando uno sviluppo \\efficace e coerente tra i membri. \end{tabular} \\
            \hline
            Analisti & \begin{tabular}[c]{@{}c@{}} Sono responsabili della analisi \\dei requisiti. Il loro compito è dominare la complessità del \\progetto con un'analisi granulare di tutte le funzionalità. \end{tabular} \\
            \hline
            Progettisti & \begin{tabular}[c]{@{}c@{}} Sono responsabili della progettazione \\dell'architettura del sistema e delle componenti software\\, assicurando che siano conformi ai requisiti definiti. \end{tabular} \\
            \hline
            Sviluppatori & \begin{tabular}[c]{@{}c@{}} Si occupano della progettazione, implementazione\\ e test del software, seguendo le linee guida \\stabilite nel Way of Working. \end{tabular} \\
            \hline
            Verificatori & \begin{tabular}[c]{@{}c@{}} eseguono i test definiti nel Piano di \\Qualifica per verificare che il software soddisfi i \\requisiti e gli obiettivi di qualità. \end{tabular} \\
            \hline
        \end{tabular}
        \end{spacing}
    \end{adjustwidth}
...
Il fornitore deve seguire una serie di procedure per studiare correttamente il prodotto da sviluppare, al fine di garantire una comprensione approfondita dei requisiti e delle aspettative del committente.
...
\begin{enumerate}
    ...
    \item \textbf{Produzione del documento Analisi dei requisiti}: redazione di un documento che raccolga in modo chiaro e dettagliato tutte le informazioni emerse durante l’analisi dei requisiti, organizzando i casi d’uso individuati e i relativi requisiti associati.
    \item \textbf{Esplorazione delle tecnologie}: produzione di un prodotto software che mostri le tecnologie che il team ha identificato come quelle più adatte per soddisfare
\end{enumerate}
...
del progetto. Ogni richiesta nasce come \glossario{issue} DIPR tracciata su \glossario{Jira}, collegata allo \glossario{SCIs} coinvolto tramite gli \glossario{Smart Commit} descritti
...
Correzione di difetti segnalati da \glossario{pipeline CI/CD}, verifiche del Piano di Qualifica o feedback della proponente.
...
membri del gruppo e promuovere il principio Agile di continuo
...
\begin{itemize}
    ...
    \item GitHub per la gestione delle versioni della documentazione di progetto.
\end{itemize}
...
milestone. Questo passaggio dovrebbe risultare puramente formale.
...
strumenti:
\begin{itemize}
    \item GitHub, in particolare integrato in VSCode per la gestione delle versioni e delle \glossario{\textit{Pull Request (PR)}}, comodamente nell'ambiente di sviluppo di VSCode \ref{vscode}.
    ...
    \item Jira: strumento di gestione delle attività di progetto, utilizzato per tracciare le attività di documentazione e assegnarle ai membri del gruppo.
