...
\begin{table}[H]
    \begin{adjustwidth}{-4cm}{-4cm}
        ...
        \begin{tabular}{|c|c|c|c|c|}
            ...
            \\
             \hline
            0.5               & 1/12/2025                                                                                                                                                                 & Soligo Lorenzo  & Rocco Matteo A.       & \begin{tabular}[c]{@{}c@{}} Aggiornamento nuovo standard per gestione branch, \\ convenzioni sui nomi, date e versioni. \\ Sezioni 3.1.2, 3.1.3, 3.2.1.1.1, 3.2.4, 4.2.3.2.1,  \end{tabular}       \\
            ...
            \hline
            0.3               & 25/11/2025                                                                                                                                                                 & Soligo Lorenzo  & Fattoni Antonio
...
mantenere aggiornate le presenti norme ed eventuali riferimenti ad altri
...
\begin{itemize}
    ...
    \item VSCode come IDE di riferimento.
\end{itemize}
...
Per una più efficiente scrittura dei documenti, soprattutto di tutti quei documenti
...
Risulta quindi \textbf{obbligatorio} l'utilizzo di Smart Commit per tutti i commit, compresi quelli di Pull Request. Maggiori dettagli in \ref{smartcommit}.
...
\begin{enumerate}
    ...
    \item Quando è necessario \textbf{effettuare un commit} è obbligatorio utilizzare lo
          Smart Commit. Maggiori dettagli in \ref{Jira}.
    \item Una volta completata la task, è necessario \textbf{creare la PR} verso la
          ...
          membro del gruppo designato.
          
          Per esempio, una volta completata la stesura di
...
\begin{itemize}
    \item Tutti i file dei documenti devono seguire la regola del \textbf{Camel Case}, ovvero devono essere scritti senza spazi e con la prima lettera di ogni parola in maiuscola.
    \item All'interno dei documenti, la data deve essere riportata nel formato \textbf{YYYY-MM-DD} (anno-mese-giorno).
\end{itemize}
Per quanto riguarda la denominazione dei file sul sito, si seguono le stesse regole, aggiungendo però il numero di versione alla fine del nome del file. Per le regole di versionamento si rimanda a \ref{versione}.
...
viene effettuata da un membro del gruppo diverso dall'autore del documento.
...
principale \textit{main}, base per la versione ufficiale di rilascio corrispondente alla
\glossario{milestone}. Questo passaggio dovrebbe risultare puramente formale.
Nondimeno è il garante della qualità del documento, quindi deve impiegare il
...
\begin{enumerate}
    ...
    \item Definizione delle scadenze e del \glossario{Time Estimate} per la gestione del
          carico di lavoro.
\end{enumerate}
È quindi necessaria una specificazione sulla struttura di un Work Item. Ogni Work Item deve consistere in una fase produttiva e una fase di revisione, per garantire la qualità del prodotto finale.
...
\end{itemize}
Questo approccio divide chiaramente le responsabilità e le attività, mantenendo la correlazione tra produzione e supervisione e inoltre facilita il monitoraggio dello stato di avanzamento, la gestione delle revisioni e il conteggio di \glossario{Ore produttive} consumate.
Maggiori dettagli in \ref{metrichediProgetto}.
...
documentazione, il Team adotta il seguente standard per la denominazione delle
branch in GitHub:
\begin{itemize}
    \item La creazione della branch deve avvenire preferibilmente in modo automatico
...
Questa attività norma il modo in cui le modifiche di avanzamento vengono registrate e
...
\begin{itemize}
    ...
    \item Data della modifica (maggiori dettagli in \ref{convenzioniNomiDate}).
...
particolare per l'integrazione con Jira. Vengono quindi descritti gli Smart
...
\section{Metriche della qualità} \label{metrichediProgetto}
