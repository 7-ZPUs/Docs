...
La verifica è un processo il cui scopo è l'allineamento del software prodotto ai requisiti generati dalle aspettative della proponente.
Per evitare possibili derive del software ad ogni task vengono assegnati uno o più verificatori. In base al tipo di task i verificatori andranno ad adoperare un insieme di tecniche e pratiche consone per analizzare se il prodotto è conforme agli standard di qualità.
All'interno del Piano di Qualifica vengono elencate le attività che i verificatori devono fare al completamento della task a loro assegnatali. Troviamo anche per ogni attività gli obbiettivi, gli esiti desiderabili e quelli ottenuti.
...
L'analisi dinamica prevede l'esecuzione dell'oggetto di verifica. Durante la sua esecuzione possiamo eseguire dei test, essi dovranno essere ripetibili e automatizzabili: ripetibili perché non vogliamo evitare la regressione del software, automatizzati (perché è più efficiente) tramite Driver (permette di invocare il componente da testare se quest'ultimo non ha ancora chi lo chiama),  Stub (permette di evitare le chiamate ad altri moduli che non vogliamo testare) e Logger (permette di tracciare i test).
Vediamo le tipologie di test più comuni:
\begin{itemize}
    \item Unità: verificano il singolo componente in isolamento. Si suddividono in funzionali e strutturali, i primi si basano sulle specifiche funzionali e non sull'implementazione, cioè verificano il comportamento dell'unità in base alle specifiche, i secondi invece sulla struttura interna del codice, come ad esempio la copertura dei rami.
    \item Test di regressione: verificano che modifiche o correzioni non introducano nuovi errori in parti del codice già verificate. Ripetiamo i test già eseguiti. 
    \item Test di integrazione: verifichiamo che più unità già testate singolarmente possano essere integrate, cioè interagiscano correttamente.
    \item Test di sistema: verificano la totalità del sistema rispetto ai requisiti. Andiamo a collaudare il sistema con il committente, questo fa parte del processo di validazione, trattato nella sezione successiva
\end{itemize}

\subsection{Processo di validazione}
La validazione è un processo che prevede di controllare se i requisiti e le aspettative della proponente sono state soddisfatte tramite apposti test di accettazione. Questi test permettono di simulare uno scenario realistico, in modo da dimostrare alla proponente il prodotto e ricevere il suo feedback.
