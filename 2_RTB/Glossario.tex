...
\begin{itemize}
    ...
    \item \textbf{Analista}: Ruolo che si occupa della scrittura dell’analisi dei requisiti e del piano di
    qualifica.
    ...
    \item \textbf{Amministratore}: Ruolo che si occupa del corretto svolgimento del progetto e la scrittura delle norme di progetto.
...
\begin{itemize}
    ...
    \item \textbf{Cruscotto di qualità}: Dashboard che si specializza su tutte quelle metriche che riguardano la qualità del software, come la copertura del codice, la complessità, la presenza di bug o vulnerabilità, e altri indicatori di qualità. Per Dashboard più generiche, si veda la voce "Dashboard/Cruscotto".
...
\end{itemize}
\begin{itemize}
    \item \textbf{DIPReader}: è il software commissionato dalla proponente su cui si concentra il progetto
\end{itemize}
\section{E}
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Framework}: Un insieme di strumenti, librerie e best practice che forniscono una struttura di base per lo sviluppo di software. I framework aiutano a semplificare il processo di sviluppo, fornendo soluzioni predefinite per problemi comuni e facilitando la gestione del codice.
    \item \textbf{Fornitore}: Entità che si occupa di fornire il prodotto software alla proponente, seguendo le specifiche e i requisiti definiti. Il fornitore è responsabile dello sviluppo, della consegna e del supporto del prodotto software, lavorando in collaborazione con la proponente per garantire il successo del progetto.
\end{itemize}
\section{G}
\begin{itemize}
...
\section{P}
\begin{itemize}
    \item \textbf{Processo}: Un insieme di attività correlate e coordinate che trasformano input in output, con l'obiettivo di raggiungere un risultato specifico. I processi governano le attività di progetto, organizzando e strutturando il lavoro in modo efficiente.
    \item \textbf{Product Baseline (PB)}: un’istantanea (snapshot) del progetto al termine delle attività di pianificazione con il progetto approvato dal committente.
    \item \textbf{Programmatore}: Ruolo che si occupa di codificare il progetto e i rispettivi test.
    \item \textbf{Progettista}: Ruolo che si occupa di creare un progetto basandosi sullo studio dei requisiti degli analisti.
    \item \textbf{Proof of Concept (PoC)}: Prodotto software che dimostra la fattibilità di un'idea o di una tecnologia specifica. Il PoC è spesso utilizzato per testare concetti, validare ipotesi o dimostrare la funzionalità di una soluzione prima di investire risorse significative nello sviluppo completo del prodotto.
    ...
    \item \textbf{Pipeline CI/CD}: Una pipeline CI/CD è un insieme di processi automatizzati che consentono di integrare, testare e distribuire il software in modo continuo.
...
\begin{itemize}
    ...
    \item \textbf{Responsabile di progetto}: Ruolo che si occupa di gestire la suddivisione delle attività tra i membri, della scrittura dei verbali, della scrittura del piano di progetto e della comunicazione con la proponente.
    \item \textbf{Requirements and Tecnology Baseline (RTB)}: Converge l'Analisi dei Requisiti con la risposta Tecnica. Una volta compresi i vincoli tecnologici emerge la necessità che le tecnologie rispecchiano tali vincoli, pesando la scelta in base all'impatto che tali tecnologie hanno sul design (1° Compito della RTB). L'uso di più tecnologie pretende anche che queste eterogenicità possano convivere in un Proof Of Concept (2° Compito della RTB).
\end{itemize}
\section{S}
\begin{itemize}
    ...
    \item \textbf{Smart-commit}: Un commit formattato in modo specifico che include un riferimento a una issue Jira, lo stato della issue, il tempo per completarla e un messaggio descrittivo. Gli smart-commit sono fondamentali per automatizzare il tracciamento delle issue e migliorare la gestione del progetto in Jira.
    ...
    \item \textbf{Stato di Avanzamento dei Lavori (SAL)}: Controllo dei progressi effettuati in un determinato frangente di tempo. La progressione dei lavori viene comunicata in riunioni periodiche tra il gruppo e la proponente.
...
\section{U}
\begin{itemize}
    \item \textbf{Use Case}: Vedi \textbf{Caso d'Uso}.
