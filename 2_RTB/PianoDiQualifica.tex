...
Strettamente collegato, nonché conseguenza del CPI, è l'andamento di EAC e BAC. Tra le settimane 3 e 5 notiamo una diminuzione, che tuttavia non tiene conto della sottostima delle attività. Questo dovuto a quanto detto sopra, ovvero che nelle attività completate il costo reale è stato inferiore. Successivamente invece si può notare un incremento dell EAC, sintomo dell'aumento dei costi. Questo aumento è dovuto alle numerose iterazioni (talvolta molto distruttive) sul documento di Analisi dei Requisiti. A fronte degli incontri col prof. Cardin infatti, il gruppo ha dovuto rivedere quasi completamente la struttura degli Use Case\textsubscript{\textit{G}}. \\
\includegraphics[width=\textwidth]{../assets/pdq/ev_pv_ac_eac_etc.png}
Si può apprezzare dal grafico quanto detto fino ad ora, con un aumento dell EAC nella fase iniziale, con un suo successivo rientro. \\
\includegraphics[width=\textwidth]{../assets/pdq/gfi.png}
Il gruppo ha deciso di misurare il Gulpease Index per garantire un buona leggibilità dei documenti, in particolar modo quelli rivolti ad esterni. Nonostante nelle fasi iniziali il gruppo non abbia posto particolare attenzione a questa metrica è comunque riuscito ad ottenere buoni risultati. Unica eccezione sono le Norme di Progetto, che a differenza degli altri documenti sono di natura più descrittiva e con meno punti elenco. Tuttavia, per garantire un limite inferiore è stato implementato durante le settimane 7-10 (SPRINT 4), un controllo automatico. Questo controllo garantisce che ciò che confluisce nella repo documentale abbia un indice di Gulpease di almeno 60. \\
...
Il gruppo ha incontrato due volte in due sprint diversi dei rischi non previsti. La prima volta, compatibilmente con i dati mostrati precedentemente, è dovuta alla sottostima delle attività. Nello sprint successivo invece si è verificato un rischio di natura non prevedibile, indisponendo uno dei membri del gruppo. \\
\includegraphics[width=\textwidth]{../assets/pdq/rsi.png}
Come si evince dal grafico, la stabilità dei requisiti è rimasta costante per la fase iniziale del progetto, dove i requisiti ancora non erano stati esplorati. Questo dovuto al fatto che il gruppo era impegnato nella definizione delle Norme di Progetto. Successivamente si è verificato un calo non indifferente, ma non inaspettato. Durante questo periodo infatti l'attenzione si è spostata sullo studio del dominio di utilizzo del software e sulle aspettative della proponente. La poca conoscenza del dominio di applicazione e la continua selezione e ideazione dei requisiti, scelti anche in base alla fattibilità, ha portato a questa instabilità. Si sottolinea che la proponente non ha mai imposto vincoli ferrei sulle funzionalità del prodotto, motivo per cui i requisiti sono stati man mano proposti e discussi. Sempre per il motivo appena detto, non è quasi mai stato necessario modificare, aggiungere o eliminare requisiti. Questo ha portato ad un successivo stabilizzarsi dell'indice nella fase finale.
\vfill
\begin{flushright}
    \textit{7-ZPUs}
\end{flushright}
